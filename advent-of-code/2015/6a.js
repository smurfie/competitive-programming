  ...
  function solve(input) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (command.split(" ")[0] == "toggle") {
        x1 = parseInt(command.split(" ")[1].split(",")[0]);
        y1 = parseInt(command.split(" ")[1].split(",")[1]);
        x2 = parseInt(command.split(" ")[3].split(",")[0]);
        y2 = parseInt(command.split(" ")[3].split(",")[1]);
        option = 0;
      } else {
        x1 = parseInt(command.split(" ")[2].split(",")[0]);
        y1 = parseInt(command.split(" ")[2].split(",")[1]);
        x2 = parseInt(command.split(" ")[4].split(",")[0]);
        y2 = parseInt(command.split(" ")[4].split(",")[1]);
        option = command.split(" ")[1] == "on" ? 1 : 2;
      }

      for (var j = x1; j <= x2; j++) {
        for (var k = y1; k <= y2; k++) {
          if (lights[j][k] && option != 1) lights[j][k] = false;
          else if (!lights[j][k] && option != 2) lights[j][k] = true;
        }
      }
    }
