(() => {
  function solve(input, totalTime) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      var speed = parseInt(line[3]);
      var time = parseInt(line[6]);
      var wait = parseInt(line[13]);

      var lap = time + wait;
      var nLaps = Math.floor(totalTime / lap);
      var seconds = Math.min(totalTime % lap, time) + nLaps * time;
