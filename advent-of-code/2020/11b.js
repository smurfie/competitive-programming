(() => {
  function solve(input) {
    ...
    while (m.hash() != hash) {
      hash = m.hash();
      var m2 = Utils.duplicate(m.m);
      for (var i = 0; i < m.h; i++) {
        for (var j = 0; j < m.w; j++) {
          var el = m.m[i][j];
          if (el != ".") {
            var adjOcc = countOcc(m, i, j);
            m2[i][j] =
              (el == "L" && adjOcc == 0) || (el == "#" && adjOcc < 5)
                ? "#"
                : "L";
          }
        }
  ...
  function countOcc(m, i, j) {
    var count = 0;
    var i1 = i - 1;
    var j1 = j - 1;
    while (i1 >= 0 && j1 >= 0 && m.m[i1][j1] == ".") {
      i1--;
      j1--;
    }
    if (i1 >= 0 && j1 >= 0 && m.m[i1][j1] == "#") count++;

    i1 = i - 1;
    j1 = j;
    while (i1 >= 0 && m.m[i1][j1] == ".") {
      i1--;
    }
    if (i1 >= 0 && m.m[i1][j1] == "#") count++;

    i1 = i - 1;
    j1 = j + 1;
    while (i1 >= 0 && j1 < m.w && m.m[i1][j1] == ".") {
      i1--;
      j1++;
    }
    if (i1 >= 0 && j1 < m.w && m.m[i1][j1] == "#") count++;

    i1 = i;
    j1 = j - 1;
    while (j1 >= 0 && m.m[i1][j1] == ".") {
      j1--;
    }
    if (j1 >= 0 && m.m[i1][j1] == "#") count++;

    i1 = i;
    j1 = j + 1;
    while (j1 < m.w && m.m[i1][j1] == ".") {
      j1++;
    }
    if (j1 < m.w && m.m[i1][j1] == "#") count++;

    i1 = i + 1;
    j1 = j - 1;
    while (i1 < m.h && j1 >= 0 && m.m[i1][j1] == ".") {
      i1++;
      j1--;
    }
    if (i1 < m.h && j1 >= 0 && m.m[i1][j1] == "#") count++;

    i1 = i + 1;
    j1 = j;
    while (i1 < m.h && m.m[i1][j1] == ".") {
      i1++;
    }
    if (i1 < m.h && m.m[i1][j1] == "#") count++;

    i1 = i + 1;
    j1 = j + 1;
    while (i1 < m.h && j1 < m.w && m.m[i1][j1] == ".") {
      i1++;
      j1++;
    }
    if (i1 < m.h && j1 < m.w && m.m[i1][j1] == "#") count++;
...
L.LLLLL.LL`,
  ...
  dataset.push({
    input: `LLLLLLLL.LLLLLLLLLLLLL.LLLLL.LLLLLLLL.LLLLLLLLLLLLLLLL.LLLL..LLLLLLL..LLLLLL.LLLLLLLL.L.LLLLLLLLLLL
