(() => {
  function solve(input) {
    ...
    for (var line of lines) {
      var outer = line.split(" bags contain ")[0];
      var inners = line.split(" bags contain ")[1].split(", ");
      var obj = {};
      for (var inner of inners) {
        if (!inner.startsWith("no")) {
          var n = parseInt(inner.split(" ")[0]);
          var bag = inner.split(" ")[1] + " " + inner.split(" ")[2];
          obj[bag] = n;
          if (!dictInv[bag]) dictInv[bag] = [];
          if (dictInv[bag].indexOf(outer) == -1) dictInv[bag].push(outer);
        }
      }
      dict[outer] = obj;
    }
    ...
    var containers = {};
    var sum = 0;
    var state = dictInv["shiny gold"];
    while (state.length > 0) {
      var bag = state.pop();
      if (!containers[bag]) {
        containers[bag] = true;
        sum++;
        (dictInv[bag] || []).forEach((element) => {
          state.push(element);
        });
      }
    }
