(() => {
  ...
  function evaluate(line, i) {
    var first;
    if (line[i] == "(") {
      [first, i] = evaluate(line, i + 1);
    } else {
      first = BigInt(line[i]);
    }
    i++;
    while (i < line.length && line[i] != ")") {
      ...
      var second;
      if (prod) {
        [second, i] = evaluate(line, i);
      } else if (line[i] == "(") {
        [second, i] = evaluate(line, i + 1);
        i++;
      } else {
        second = BigInt(line[i]);
        i++;
      }
      first = prod ? first * second : first + second;
    }
    return [first, i];
  }
