(() => {
  ...
  function check(obj) {
    ...
    for (var el of props) if (obj[el] == undefined) return false;

    var byr = parseInt(obj["byr"]);
    if (byr < 1920 || byr > 2002) return false;

    var iyr = parseInt(obj["iyr"]);
    if (iyr < 2010 || iyr > 2020) return false;

    var eyr = parseInt(obj["eyr"]);
    if (eyr < 2020 || eyr > 2030) return false;

    var hgt = obj["hgt"];
    if (hgt.endsWith("cm")) {
      hgt = parseInt(hgt.substring(0, hgt.length - 2));
    ...
    var hcl = obj["hcl"];
    if (hcl.match(/^#[0-9a-f]{6}$/i) == undefined) return false;
    ...
    var pid = obj["pid"];
    if (pid.match(/^[0-9]{9}$/i) == undefined) return false;
