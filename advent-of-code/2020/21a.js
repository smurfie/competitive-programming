(() => {
  function solve(input) {
    ...
    var times = {};
    var sets = {};
    var finalIngredients = new Set();

    for (var line of lines) {
      var ingredients = line.split(" (contains ")[0];
      var allergens = line.split(" (contains ")[1];
      allergens = allergens.substring(0, allergens.length - 1);
      ingredients = ingredients.split(" ");
      allergens = allergens.split(", ");
      for (var ingredient of ingredients) {
        times[ingredient] = times[ingredient] || 0;
        times[ingredient]++;
      }
      for (var allergen of allergens) {
        var newSet = new Set(ingredients);
        if (!sets[allergen]) sets[allergen] = newSet;
        else sets[allergen] = intersection(sets[allergen], newSet);
      }
    }

    for (var key in sets) {
      for (var el of sets[key]) {
        finalIngredients.add(el);
      }
    }

    var count = 0;
    for (var key in times) {
      if (!finalIngredients.has(key)) count += times[key];
    }
...
sqjhc mxmxvkd sbzzf (contains fish)`,
  ...
  dataset.push({
    input: `bbddn gkgvg csspt srm lffgc dpkvsdk gvcn vbk zhvfrl rnnsn xmmpt zltlhp zmzq jrgsgfb jdbcr vnvxb dzntt pjzmvrq mnrjrf nmxbgzp jbvhc pmvjg vgbrv hvtc vbkdkkfg cxjqxbt mxpsg bhsj jpqsk lhbj tzsr drbq kjgl bqprg nlrtm dfkmmb (contains peanuts, sesame, shellfish)
