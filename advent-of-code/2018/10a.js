(() => {
  function solve(input, output) {
    ...
    for (var line of lines) {
      ...
      var velx = parseInt(numbers[4]);
      var vely = parseInt(numbers[5]);
      for (var i = 0; i < points.length; i++) {
        if (Math.abs(vely - points[i].vely) > 1) {
          mean += Math.abs((y - points[i].y) / (vely - points[i].vely));
          countMean++;
        }
      }
      points.push({ x, y, velx, vely });
    }

    var time = Math.round(mean / countMean);
    var minx = Infinity;
    var miny = Infinity;
    var maxx = 0;
    var maxy = 0;
    ...
    for (var i = 0; i < points.length; i++) {
      var x = points[i].x + points[i].velx * time;
      var y = points[i].y + points[i].vely * time;
    ...
    var matrix = Utils.matrix(maxy - miny + 1, maxx - minx + 1, ".");
    for (var i = 0; i < points.length; i++) {
      var x = points[i].x + points[i].velx * time - minx;
      var y = points[i].y + points[i].vely * time - miny;
      matrix[y][x] = "#";
    }

    // Enable this code to se the solution printed in the console
