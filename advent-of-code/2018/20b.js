(() => {
  ...
  function solve(input) {
    ...
    while (states.length > 0) {
      ...
      steps = visited[state];
      for (var path of dict[state]) {
        if (!visited[path] && visited[path] != 0) {
          visited[path] = steps + 1;
          if (steps + 1 >= 1000) paths++;
          states.push(path);
        }
      }
    }
  ...
  function lab(s) {
    switch (word[i]) {
      case "(":
        ...
        while (word[i] != ")") {
          i++;
          for (var el of lab(s)) states.add(el);
        }
        i++;
        return lab(states);
      case ")":
    ...
    var s2 = new Set();
    for (var obj of s) {
      var x = parseInt(obj.split(",")[0]);
      var y = parseInt(obj.split(",")[1]);
      var iniX = x;
      var iniY = y;
  ...
  function addDoor(x1, y1, x2, y2) {
    var hash = x1 + "," + y1;
    if (!dict[hash]) dict[hash] = new Set();
    dict[hash].add(x2 + "," + y2);
  }
