(() => {
  function solve(input) {
    ...
    for (var line of lines) {
      word = line.split(" ");
      if (word[2] == "Guard") {
        guard = word[3].substring(1);
        if (!guards[guard]) guards[guard] = [];
      } else {
        guards[guard].push(parseInt(word[1].split(":")[1]));
      }
    }
    ...
    for (var key in guards) {
      var sleep = 0;
      var guard = guards[key];
      for (var i = 0; i < guard.length; i += 2) {
        sleep += guard[i + 1] - guard[i];
      }
      if (sleep > max) {
        max = sleep;
        maxGuard = key;
    ...
    var sleep = [];
    for (var i = 0; i < 60; i++) sleep[i] = 0;
    guard = guards[maxGuard];
    for (var i = 0; i < guard.length; i += 2) {
      for (var j = guard[i]; j < guard[i + 1]; j++) {
        sleep[j]++;
      }
    }
...
[1518-11-05 00:55] wakes up`,
  ...
  dataset.push({
    input: `[1518-05-12 00:46] wakes up
