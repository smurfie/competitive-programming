(() => {
  ...
  function solve(input) {
    ...
    do {
      groups = Utils.duplicate(groupsIni);
      for (group of groups) {
        if (group.type == 0) group.attack += boost;
      }
      var kills = 1;
      while (
        ...
        var totalUnits = groups.reduce((a, b) => a + b.units, 0);
        targets();
        attacks();
        kills = totalUnits - groups.reduce((a, b) => a + b.units, 0);
      }
      boost++;
    } while (groups[0].type == 1 || kills == 0);
