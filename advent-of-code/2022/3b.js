(() => {
  function solve(input) {
    ...
    for (var i = 0; i < lines.length; i += 3) {
      ...
      for (var letter of s2) {
        dict[letter] = dict[letter] && dict[letter] != "2" ? "12" : "2";
      }
      for (var letter of s3) {
        if (dict[letter] && dict[letter].length == 2) {
          if (letter === letter.toUpperCase()) {
            sum += letter.charCodeAt(0) - "A".charCodeAt(0) + 27;
          } else {
            sum += letter.charCodeAt(0) - "a".charCodeAt(0) + 1;
          }
          break;
        }
      }
