(() => {
  function solve(input, cubeSide) {
    var lines = Utils.read(input);
    var map = [];

    var maxLength = 0;
    for (var i = 0; lines[i].length > 0; i++) {
      maxLength = Math.max(maxLength, lines[i].length + 2);
    }
    map.push(Array(maxLength).fill(" "));

    var i = 0;
    for (; lines[i].length > 0; i++) {
      var line = " " + lines[i] + " ";
      var arr = line.split("");
      while (arr.length < maxLength) {
        arr.push(" ");
      }
      map.push(arr);
    }
    map.push(Array(maxLength).fill(" "));

    // Dice
    // 2
    // 1 3 6 4
    // 5
    // rot te the left: 0(0), 90(1), 180(2), 270(3)
    // Right(0), Down(1), Left(2), Up(3)
    var diceDict = {
      1: {
        0: { num: 3, rot: 0, enter: 2 },
        1: { num: 5, rot: 0, enter: 3 },
        2: { num: 4, rot: 0, enter: 0 },
        3: { num: 2, rot: 0, enter: 1 }
      },
      2: {
        0: { num: 3, rot: 1, enter: 3 },
        1: { num: 1, rot: 0, enter: 3 },
        2: { num: 4, rot: 3, enter: 3 },
        3: { num: 6, rot: 2, enter: 3 }
      },
      3: {
        0: { num: 6, rot: 0, enter: 2 },
        1: { num: 5, rot: 1, enter: 0 },
        2: { num: 1, rot: 0, enter: 0 },
        3: { num: 2, rot: 3, enter: 0 }
      },
      4: {
        0: { num: 1, rot: 0, enter: 2 },
        1: { num: 5, rot: 3, enter: 2 },
        2: { num: 6, rot: 0, enter: 0 },
        3: { num: 2, rot: 1, enter: 2 }
      },
      5: {
        0: { num: 3, rot: 3, enter: 1 },
        1: { num: 6, rot: 2, enter: 1 },
        2: { num: 4, rot: 1, enter: 1 },
        3: { num: 1, rot: 0, enter: 1 }
      },
      6: {
        0: { num: 4, rot: 0, enter: 2 },
        1: { num: 5, rot: 2, enter: 1 },
        2: { num: 3, rot: 0, enter: 0 },
        3: { num: 2, rot: 2, enter: 3 }
      }
    };

    var x = 1;
    var y = map[x].findIndex((i) => i == ".");
    var dir = 0; // Right(0), Down(1), Left(2), Up(3)

    var dict = {
      1: {
        x: toCubeNum(x, cubeSide),
        y: toCubeNum(y, cubeSide),
        rot: 0
      }
    };

    var diceNums = [1];
    while (diceNums.length > 0) {
      var diceNum = diceNums.shift();
      var el = dict[diceNum];
      if (
        // UP
        el.x > 0 &&
        map[cubeSide * (el.x - 1) + 1][cubeSide * el.y + 1] != " "
      ) {
        var newEl = diceDict[diceNum][(3 + el.rot) % 4];
        if (!dict[newEl.num]) {
          diceNums.push(newEl.num);
          dict[newEl.num] = {
            x: el.x - 1,
            y: el.y,
            rot: (el.rot + newEl.rot) % 4
          };
        }
      }
      if (
        // DOWN
        (el.x + 1) * cubeSide + 2 < map.length &&
        map[cubeSide * (el.x + 1) + 1][cubeSide * el.y + 1] != " "
      ) {
        var newEl = diceDict[diceNum][(1 + el.rot) % 4];
        if (!dict[newEl.num]) {
          diceNums.push(newEl.num);
          dict[newEl.num] = {
            x: el.x + 1,
            y: el.y,
            rot: (el.rot + newEl.rot) % 4
          };
        }
      }
      if (
        // LEFT
        el.y > 0 &&
        map[cubeSide * el.x + 1][cubeSide * (el.y - 1) + 1] != " "
      ) {
        var newEl = diceDict[diceNum][(2 + el.rot) % 4];
        if (!dict[newEl.num]) {
          diceNums.push(newEl.num);
          dict[newEl.num] = {
            x: el.x,
            y: el.y - 1,
            rot: (el.rot + newEl.rot) % 4
          };
        }
      }
      if (
        // RIGHT
        (el.y + 1) * cubeSide + 2 < map[0].length &&
        map[cubeSide * el.x + 1][cubeSide * (el.y + 1) + 1] != " "
      ) {
        var newEl = diceDict[diceNum][el.rot];
        if (!dict[newEl.num]) {
          diceNums.push(newEl.num);
          dict[newEl.num] = {
            x: el.x,
            y: el.y + 1,
            rot: (el.rot + newEl.rot) % 4
          };
        }
      }
    }
    var invDict = {};
    for (var el of Object.keys(dict)) {
      invDict[dict[el].x + "," + dict[el].y] = Number(el);
    }

    var route = lines[i + 1]
      .split(/([RL])/g)
      .map((i) => (isNaN(i) ? i : Number(i)));

    for (var el of route) {
      if (el === "R") {
        dir++;
        dir %= 4;
      } else if (el === "L") {
        dir += 3;
        dir %= 4;
      } else {
        while (el--) {
          var xDice = toCubeNum(x, cubeSide);
          var yDice = toCubeNum(y, cubeSide);
          var numDice = invDict[xDice + "," + yDice];

          switch (
            dir // Right(0), Down(1), Left(2), Up(3)
          ) {
            case 0:
              var tmpx = x;
              var tmpy = y + 1;
              var tmpDir = dir;
              if (map[tmpx][tmpy] === " ") {
                var newDice = diceDict[numDice][(dir + dict[numDice].rot) % 4];
                var posNewDice = dict[newDice.num];
                var enter = (4 + newDice.enter - posNewDice.rot) % 4;
                var xOffset = (x - 1) % cubeSide;
                switch (enter) {
                  case 0:
                    tmpx = (posNewDice.x + 1) * cubeSide - xOffset;
                    tmpy = (posNewDice.y + 1) * cubeSide;
                    tmpDir = 2;
                    break;
                  case 1:
                    tmpx = (posNewDice.x + 1) * cubeSide;
                    tmpy = 1 + posNewDice.y * cubeSide + xOffset;
                    tmpDir = 3;
                    break;
                  case 2:
                    tmpx = 1 + posNewDice.x * cubeSide + xOffset;
                    tmpy = 1 + posNewDice.y * cubeSide;
                    tmpDir = 0;
                    break;
                  case 3:
                    tmpx = 1 + posNewDice.x * cubeSide;
                    tmpy = (posNewDice.y + 1) * cubeSide - xOffset;
                    tmpDir = 1;
                    break;
                }
              }
              if (map[tmpx][tmpy] === "#") {
                el = 0;
              } else {
                x = tmpx;
                y = tmpy;
                dir = tmpDir;
              }
              break;
            case 1:
              var tmpx = x + 1;
              var tmpy = y;
              var tmpDir = dir;
              if (map[tmpx][tmpy] === " ") {
                var newDice = diceDict[numDice][(dir + dict[numDice].rot) % 4];
                var posNewDice = dict[newDice.num];
                var enter = (4 + newDice.enter - posNewDice.rot) % 4;
                var yOffset = (y - 1) % cubeSide;
                switch (enter) {
                  case 0:
                    tmpx = 1 + posNewDice.x * cubeSide + yOffset;
                    tmpy = (posNewDice.y + 1) * cubeSide;
                    tmpDir = 2;
                    break;
                  case 1:
                    tmpx = (posNewDice.x + 1) * cubeSide;
                    tmpy = (posNewDice.y + 1) * cubeSide - yOffset;
                    tmpDir = 3;
                    break;
                  case 2:
                    tmpx = (posNewDice.x + 1) * cubeSide - yOffset;
                    tmpy = 1 + posNewDice.y * cubeSide;
                    tmpDir = 0;
                    break;
                  case 3:
                    tmpx = 1 + posNewDice.x * cubeSide;
                    tmpy = 1 + posNewDice.y * cubeSide + yOffset;
                    tmpDir = 1;
                    break;
                }
              }
              if (map[tmpx][tmpy] === "#") {
                el = 0;
              } else {
                x = tmpx;
                y = tmpy;
                dir = tmpDir;
              }
              break;
            case 2:
              var tmpx = x;
              var tmpy = y - 1;
              var tmpDir = dir;
              if (map[tmpx][tmpy] === " ") {
                var newDice = diceDict[numDice][(dir + dict[numDice].rot) % 4];
                var posNewDice = dict[newDice.num];
                var enter = (4 + newDice.enter - posNewDice.rot) % 4;
                var xOffset = (x - 1) % cubeSide;
                switch (enter) {
                  case 0:
                    tmpx = 1 + posNewDice.x * cubeSide + xOffset;
                    tmpy = (posNewDice.y + 1) * cubeSide;
                    tmpDir = 2;
                    break;
                  case 1:
                    tmpx = (posNewDice.x + 1) * cubeSide;
                    tmpy = (posNewDice.y + 1) * cubeSide - xOffset;
                    tmpDir = 3;
                    break;
                  case 2:
                    tmpx = (posNewDice.x + 1) * cubeSide - xOffset;
                    tmpy = 1 + posNewDice.y * cubeSide;
                    tmpDir = 0;
                    break;
                  case 3:
                    tmpx = 1 + posNewDice.x * cubeSide;
                    tmpy = 1 + posNewDice.y * cubeSide + xOffset;
                    tmpDir = 1;
                    break;
                }
              }
              if (map[tmpx][tmpy] === "#") {
                el = 0;
              } else {
                x = tmpx;
                y = tmpy;
                dir = tmpDir;
              }
              break;
            case 3:
              var tmpx = (x - 1) % map.length;
              var tmpy = y;
              var tmpDir = dir;
              if (map[tmpx][tmpy] === " ") {
                var newDice = diceDict[numDice][(dir + dict[numDice].rot) % 4];
                var posNewDice = dict[newDice.num];
                var enter = (4 + newDice.enter - posNewDice.rot) % 4;
                var yOffset = (y - 1) % cubeSide;
                switch (enter) {
                  case 0:
                    tmpx = (posNewDice.x + 1) * cubeSide - yOffset;
                    tmpy = (posNewDice.y + 1) * cubeSide;
                    tmpDir = 2;
                    break;
                  case 1:
                    tmpx = (posNewDice.x + 1) * cubeSide;
                    tmpy = 1 + posNewDice.y * cubeSide + yOffset;
                    tmpDir = 3;
                    break;
                  case 2:
                    tmpx = 1 + posNewDice.x * cubeSide + yOffset;
                    tmpy = 1 + posNewDice.y * cubeSide;
                    tmpDir = 0;
                    break;
                  case 3:
                    tmpx = 1 + posNewDice.x * cubeSide;
                    tmpy = (posNewDice.y + 1) * cubeSide - yOffset;
                    tmpDir = 1;
                    break;
                }
              }
              if (map[tmpx][tmpy] === "#") {
                el = 0;
              } else {
                x = tmpx;
                y = tmpy;
                dir = tmpDir;
              }
              break;
          }
        }
      }
    }

    return 1000 * x + 4 * y + dir;
  }

  function toCubeNum(x, cubeSide) {
    return Math.floor((x - 1) / cubeSide);
  }

  var dataset = [];

  dataset.push({
    input: `        ...#
        .#..
        #...
        ....
...#.......#
........#...
..#....#....
..........#.
        ...#....
        .....#..
        .#......
        ......#.

10R5L5R10L4R5L5`,
    cubeSide: 4,
    output: 5031
  });

  dataset.push({
    input: `                                                  ........#...........................#............#............#..............#.....##...............
                                                  .#..#...............................#..........##.#.........................#............#..........
                                                  .......#...##...#..........#........................#.........#......................#..............
                                                  .............................#..........#..#.#..........#...................#....................#..
                                                  ...........#...........#...........#.......#....................#.......#........#..................
                                                  #..#..........#........#..........................#....#.#..........................................
                                                  .....#.............................#..................#......................#..#.....#.....#.#.....
                                                  ............#....##...#.................#..#..........#............##.................#.....#.......
                                                  ...........#..........................#....................#........#.#........#.#..................
                                                  .#.#.##........#.......#..........#.#.....#..........#.#..#.....................#...#.#.............
                                                  .........................#...#......#.#...#.....#.##.........#............#.........................
                                                  ....#..#............................#.............##........#......#...#..#..........#..............
                                                  ............#..............#.......#.....#.#.#.......#.#............##......#..#...........#.......#
                                                  ...........#.......#................................#.............#.......#.#.#........#...........#
                                                  .................#....................#........#...............#...........#.....#....#......#....#.
                                                  ##.....#.#......#..#................#..#...#...#....#.....#..#..............#.#.......##.....#......
                                                  ........................#.......#..#.......#..........#...............................#........###..
                                                  .#.#.....#.....................#................#.#...#..###.....#..###..........#............#...##
                                                  ...#.#..#..#.#......................##....#........#..........#..............#.....................#
                                                  ................................#.##..#............#........#......................#..#....#........
                                                  .................#.......#........................................#...#..........#...#...#.#........
                                                  .#..............#......#..........##.#............#...........#....#..............#.......#......#..
                                                  .................#.#..#......#..#......###........#......................#....#.......#....#......#.
                                                  #.........#..##.....#...........#...#.....#....................#..#..........#......................
                                                  ..##........##....#..#...........#.....#.....#.......................#.....#....#..............#....
                                                  ..............#.#..........#...#..................#..#....................#........#..........##....
                                                  ...#..................................#...#...#..........##.....#...........#..#..........#.........
                                                  ....................##.......#......#........#..#....................#.....#........................
                                                  ...#.#......#.#..........#...........##.....................................#...##.............#....
                                                  ..#.....................#.................#...#........................................#...........#
                                                  .#....##...#.......#.......#...........................#............##.........#...#................
                                                  ..#..#.#...................#......................#.........#...#.....#.............................
                                                  .......#..#..#..........#......#...........#........#....#...........................#.........#....
                                                  ...##..........#....#.............#..#.......................#...................##............#....
                                                  #.....#........##.....#.....#........#....#......#.#..#.#.#.........#..........#........#......#..#.
                                                  .............#........#........................#.......#....#...#.#......#......#...........#.......
                                                  ...................#.#...............#........#..............................#...#...............#..
                                                  ......#.....#...##..#.........#...#.........#.#.............................#...............#..#.#..
                                                  ....#....#..........#....#....#...............##..................##...................#..........#.
                                                  ...........#............#.............#..................................#..........#.....#..#......
                                                  .................#.....#...#......#..#..#....................#.....#......#....#....................
                                                  ..................#.........#.#..#......#...........#................................#........#.....
                                                  ..............#...#...#........................................#.......#............##....#.........
                                                  ...#..........#............#........................#......#...................#.#.........#..#.#...
                                                  ..#...........#........#.........#......#.....##..............#...........#...#.....................
                                                  ...................#......#.....#..............................................#..................#.
                                                  ..#.......................#......#.......#.#.........#...#...........................#..............
                                                  .....#.........#.........#.............#.....#.....#........##..........#...#.#.....................
                                                  ..#....#...............#......#...#.......#.#.........###..#.##......................#...##....#....
                                                  ..........#......#.................###.#..............#......#............#....#.......#....#.......
                                                  ...........#........#.............................
                                                  ......#.#..........#.....#..#................#....
                                                  ........##..............#.....#...#...............
                                                  ............#....##...#..................#.#.....#
                                                  ........#.......#.......................#.........
                                                  ..#....#..........#............#................#.
                                                  .....#....#...##.......#..........................
                                                  .##..........#.....#..#.......#.............#.....
                                                  .........................#....#.#..#..............
                                                  .........................#......#.................
                                                  .....#.......#..............#......#.....#..##.##.
                                                  ..................................#.#..#..........
                                                  .#...............#..........#......#..............
                                                  #.#...#.............#....##..#...#.....#......#...
                                                  ..................#......#...#....................
                                                  ........#.....#...#.......................#...#...
                                                  #.#.#......#..##...#............#.............#...
                                                  #.............#.......................#...........
                                                  .............#.........#....#....#..#...#...##..#.
                                                  .....#....#........#........#...#.................
                                                  .............#....#..........#............#...#...
                                                  .#......##..#........#....#.#....#............#...
                                                  .....#..#....................................#....
                                                  ............#.....##.........#.........#..........
                                                  ..................................................
                                                  ...........#.#.............##.....................
                                                  .#.#.........#.......#......#...........#.........
                                                  .....#.....#............#.....###.................
                                                  .........##.............#.#..#....#......###......
                                                  ....#...............#....#...#........#....#......
                                                  ....#.......##....#...........#...................
                                                  .......#..#.............#.#.......#.....#.#.......
                                                  ........#.........................................
                                                  ..............................#................#..
                                                  ....#.#............#...#..#......#......#..#..#...
                                                  ...#..##..........#.#......................##....#
                                                  .......#.....#.#..............#.#......#......#...
                                                  ...............#.........##........#.......#......
                                                  #......#..#....#.....##...........................
                                                  #.....##.........#......#.#..##...................
                                                  ......#...#...#....#....#.........................
                                                  ...#.........#...##.............#.#.#.............
                                                  ...............#...#.......#.....#................
                                                  ..........#...#...###.................#.#.........
                                                  ......#....#..#.............#..................#..
                                                  ..#....................#...............#........#.
                                                  ....#....#.#......#.#........#......##.....#......
                                                  .......#..........#...#...#..........#............
                                                  ....#............##....#........#.................
                                                  #..........#......................#...............
.......##...#...............................##.....#...........#.....##......................#......
.....................#............#.........#..................................##.....#...#......#.#
.........#....#..#........#.#.#.##.................#.#..#...........................................
..........##.#.................#.#...........#.................#.............#....#.............#...
............#.......................##....#...............#..#..#......#...............#......#.....
.#..........#.#...#..............#..#.........#....#....................#.....................#.....
#.......###........#....#..#...#..................#......#...........................#....#..#......
...#.#..#....#.....................#...................................#.#....................#....#
..#.#....................................................#........#..............#........#..#....##
.#..#.#....#.........#...............#.......#...#...........#.......#..#....#..#......#............
..#.#................#.....................#...............#...#......................#.............
.............#.#.#........................#.........#...#........#......#..........#..........#.....
......#........##..................#...##........................#.........#.....#..........#..#....
.#......#...#....#......#........#................#....#..................#.........#......##..#....
..###....#...#....#........#...##...........#............#...#.....#..##.#..........................
.............................................#...............#.....#.#...#........#.#...............
.......................#.#...................................#..................#...................
#............#.......................##......#..#......#..#................#.#.....#...............#
..........#....#......#............#.................#....#................#...................#....
#..#.##...............#................#.#..#....#.............#.............................#..#...
...#....................#.................#...#..#............#...#...........#.................#...
..#......#..........##....#.#...#...#................##...#.....##.#..#............#...#....#.....#.
##.....#.....#................................#.............#...#.......#.....#..#...........#..#...
..#.......#.#....#...#.....#..#..#....#....#.............#.#..........#..........#..#.....###.......
.#...........#..............#.#.........#..........#.#.....#.......#..#..........#.........#........
...............................#...................................#.....................#......#...
.............................................#.#...................#....#.....#..............#......
.#......#...#..........#..#......##.#..#............#...............#..................#...#.#......
..#........##..........................................................#...#.........#..............
.#....#...#.................#.............................#.........................................
....#........##...#................................#..#.........#.#....#........#...................
....#........#.......................#.....#....................#...#...............................
....#......#..##...................#...........##............................#..........#...........
.#........................#.............#.....#....#.....##.##..............#..............#.....#..
...............#.......#..............#....#.....#.#........................#.....#..#.#............
.........#....#....#.................................#.....#...#.....................#....#.........
.##......#.#......#.....#.........#...#..##.#..#.............#....##..#..............##......#......
.......##...#..................................#......#............#.#..#.........#.........#.#.....
#.................##.#...#..##...#.....#........................................#..#................
#................................#...#..#.......................#.#.#...................##...#......
...#......#....#.......................#..........#...................#.......................#.....
..........#....#....##...#...........#....................................................#.........
......#...........#........#..#......##................#..........................#...........#.....
............#.....#.......#.#...#.....#.......##....#.........#.#.#.......................#..#......
.........#....................#..........#..............###..................#........#.#.......#...
..............##..................#...#...#....................................#..#......#........#.
....#.#..........#..................................#.##..#.........#.........#.....................
......#.....#.......#...#..........................#..#.....#....#.#..#..#..........#...#..#.......#
......#.....................##...#.............................#....#..#.....#...........#.........#
......#..#........#....................#..........................#....#...............#............
.....#..#......................#..#...........#...
......#........#..#.....##.....#..............#...
.#.#..#...#...............###............#........
........#....................##...#.....#.........
....#....#....#.#.............#.........#..#.#...#
.........#...#..............................#....#
...........#............#...........##...#........
................#...#.......#..........#....#.....
....#.#..................###.......#....#...#.....
.......#..#...........................#.....#.....
..#..#.......#...#.........#................#...#.
.#.#.........#....................#...............
.................#...#..##..#..............#......
.....#.........#.....#..#............#.#......#..#
.................#..............#.#........#......
....#....#.##...........##...#...#.......#....#...
..#............#...#.#............................
.#..........#...........#........#.......#........
..................................................
.........#.......#................................
..........##....#..#..............................
.............#........................#...........
..#.....#.....................#..........#...#...#
.#..#...........#.................................
....#...#..#.....#.....................#...#....#.
......##......#....#........#..................#..
........#......................................#..
###..................#.....#......#........#......
#.#..........#...........#........#...............
##...#......#.....#..............#...............#
............#.......#....#..........#........#....
.........#.......................#...#............
#...............................#.....#...#..#....
.......................................#..........
.......#....##...............#......#..#...#...#..
#...#..#........#.....#....#.....#........#.......
...#..#.......#..#......#...............##........
....#.......#................................#....
.........#...................#....#......#........
#......................#....#.....................
.........#.......................##...............
............#........#..............#...#.........
....#.....#..##.....#................#............
.###...#........##........#..........#............
......#...........#...#.#...#.....................
...............#...#..............................
..##..#.......#...#.........#...#.................
................#...............#.#..#............
..........#...........##...#.........#............
.#.....#.........#..............#....#............

`,
    cubeSide: 50,
    output: 147245
  });

  Utils.check(solve, dataset, "22b");
})();
