(() => {
  function solve(input) {
    ...
    for (let line of lines) {
      ...
      let ini = parts[0].substring(0, parts[0].length - 1);
      createNode(visited, nodes, ini);

      for (let i = 1; i < parts.length; i++) {
        let end = parts[i];
        createNode(visited, nodes, end);
        nodes[ini].outgoing[end] = true;
        nodes[end].incoming[ini] = true;
      }
    }

    deleteNonReachable(nodes);

    let states = ["svr"];
    visited["svr"].paths = [1, 0, 0];

    while (states.length > 0) {
      let state = states.pop();
      let n = visited[state].paths;
      for (let k of Object.keys(nodes[state].outgoing)) {
        let node = nodes[k];
        visited[k].times++;
        if (k === "fft" || k === "dac") {
          visited[k].paths[1] += n[0];
          visited[k].paths[2] += n[1];
        } else {
          visited[k].paths[0] += n[0];
          visited[k].paths[1] += n[1];
          visited[k].paths[2] += n[2];
        }
        if (visited[k].times === Object.keys(node.incoming).length) {
          states.push(k);
        }
      }
    }
    return visited["out"].paths[2];
  }

  function createNode(visited, nodes, node) {
    if (!visited[node]) {
      visited[node] = { times: 0, paths: [0, 0, 0] };
    }
    if (!nodes[node]) {
      nodes[node] = { incoming: {}, outgoing: {} };
    }
  }

  function deleteNonReachable(nodes) {
    for (let node of Object.keys(nodes)) {
      deleteRec(nodes, node);
    }
  }

  function deleteRec(nodes, node) {
    if (
      nodes[node] &&
      node !== "svr" &&
      Object.keys(nodes[node].incoming).length === 0
    ) {
      for (let outgoing of Object.keys(nodes[node].outgoing)) {
        delete nodes[outgoing].incoming[node];
        deleteRec(nodes, outgoing);
      }
      delete nodes[node];
    }
  }
