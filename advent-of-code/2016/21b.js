(() => {
  function solve(input, word) {
    ...
    for (var line of lines.reverse()) {
      ...
      if (instruction[0] == "move") {
        word = move(word, parseInt(instruction[5]), parseInt(instruction[2]));
      }
    }
  ...
  function rotateLetter(word, x) {
    var inverseMatrix = [1, 1, 6, 2, 7, 3, 0, 4];
    var times = inverseMatrix[word.indexOf(x)];
    return rotateLeft(word, times);
  }
