(() => {
  function solve(input) {
    ...
    for (var i = 0; i < lines.length; i++) {
      var room = roomNumber(lines[i]);
      if (room != 0) {
        var name = roomName(lines[i]);
        if (name.indexOf("northpole") > -1) return room;
      }
    }
  }
  ...
  function roomName(room) {
    var name = room.split("-");
    var last = name.pop();
    var num = parseInt(last.split("[")[0]);
    name = name.join(" ");

    var realName = "";
    for (var i = 0; i < name.length; i++) {
      realName += name[i] == " " ? " " : shift(name[i], num);
    }
    return realName;
  }
  ...
  function shift(char, num) {
    return String.fromCharCode(
      ((char.charCodeAt(0) - "a".charCodeAt(0) + num) % 26) + "a".charCodeAt(0)
    );
  }
  function roomNumber(room) {
    ...
    var last = name.pop();
    var num = parseInt(last.split("[")[0]);
    var checksum = last.split("[")[1].substring(0, 5);
    name = name.join("");
    ...
    var sortedChars = Object.keys(letters)
      .map((key) => [key, letters[key]])
      .sort((a, b) => {
        var result = b[1] - a[1];
        if (result == 0) result = a[0].charCodeAt(0) - b[0].charCodeAt(0);
        return result;
      });

    return checksum == sortedChars.slice(0, 5).reduce((a, b) => a + b[0], "")
      ? num
      : 0;
  }
