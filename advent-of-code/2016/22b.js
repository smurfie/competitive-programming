(() => {
  function solve(input) {
    ...
    for (var line of lines) {
      if (line.startsWith("/")) {
        var parts = line.split(" ").filter((i) => i != "");
        var x = parseInt(parts[0].split("-")[1].substring(1));
        var y = parseInt(parts[0].split("-")[2].substring(1));
        if (!grid[x]) grid[x] = [];
        var node = {
          size: parseInt(parts[1].slice(0, -1)),
          used: parseInt(parts[2].slice(0, -1)),
          avail: parseInt(parts[3].slice(0, -1)),
          use: parseInt(parts[4].slice(0, -1))
        };
        if (node.used == 0) {
          emptyX = x;
          emptyY = y;
    ...
    // Analizing our input data we saw the only node we can move is the empty at one, lets mark the nodes
    // that can not be interchanged
    var avail = grid[emptyX][emptyY].avail;

    for (var i = 0; i < grid.length; i++) {
      for (var j = 0; j < grid[i].length; j++) {
        grid[i][j] = grid[i][j].used <= avail ? "." : "#";
      }
    }

    grid[emptyX][emptyY] = "_";
    grid[grid.length - 1][0] = "G";
    grid[0][0] = "S";

    // Uncomment next line to see the map printed
    // grid.map((i) => Utils.print(i.join("")));
    // As we can see, the path from our data (G) to the (S) is all clear, the only thing that we have to do is
    // move the empty space (_) just up the (G) and then repeat the 5 steps to get the data up one space
    // Its easy to calculate:
    return 5 * (grid.length - 2) + 45;
  }
