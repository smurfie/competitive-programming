(() => {
  function solve(input, micro1, micro2) {
    ...
    for (var line of lines) {
      if (line[0] == "v") {
        ...
        bots[bot].elements.push(value);
      } else {
        var bot = parseInt(line.split(" ")[1]);
        if (!bots[bot]) {
          bots[bot] = {
            elements: [],
            num: bot
        ...
        bots[bot].lowBot = line.split(" ")[5] == "bot";
        bots[bot].low = parseInt(line.split(" ")[6]);

        bots[bot].highBot = line.split(" ")[10] == "bot";
        bots[bot].high = parseInt(line.split(" ")[11]);
      }
    }
    ...
    for (i in bots) {
      var bot = bots[i];
      if (!bot.processed && bot.elements.length == 2) {
        num = Math.max(process(bots, bot, micro1, micro2, output), num);
      }
    }
    return num;
  }

  function process(bots, bot, micro1, micro2, output) {
    var num = -1;
    if (
      (micro1 == bot.elements[0] && micro2 == bot.elements[1]) ||
      (micro2 == bot.elements[0] && micro1 == bot.elements[1])
    )
      return bot.num;
    var min = Math.min(bot.elements[0], bot.elements[1]);
    var max = Math.max(bot.elements[0], bot.elements[1]);
    if (bot.lowBot) {
      bots[bot.low].elements.push(min);
      if (bots[bot.low].elements.length == 2)
        num = process(bots, bots[bot.low], micro1, micro2, output);
    } else output[bot.low] = min;
    if (bot.highBot) {
      bots[bot.high].elements.push(max);
      if (bots[bot.high].elements.length == 2)
        num = process(bots, bots[bot.high], micro1, micro2, output);
    } else output[bot.high] = max;
    bot.processed = true;
    return num;
  }
