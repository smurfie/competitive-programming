(() => {
  function solve(input) {
    ...
    let mat = Utils.matrix(lines.length * 2 + 1, lines[0].length * 2 + 1, ".");
    ...
    for (let i = 0; i < lines.length && x === -1; i++) {
      for (let j = 0; j < lines[i].length && x === -1; j++) {
        if (lines[i][j] === "S") {
          x = i;
          y = j;
        }
    ...
    // First move
    if (x > 0 && pipes[lines[x - 1][y]][UP]) {
      mat[2 * x][2 * y + 1] = "#";
      x--;
      lastMove = UP;
    } else if (x < lines.length - 1 && pipes[lines[x + 1][y]][DOWN]) {
      mat[2 * x + 2][2 * y + 1] = "#";
      x++;
      lastMove = DOWN;
    } else {
    ...
    while (lines[x][y] !== "S") {
      mat[2 * x + 1][2 * y + 1] = "#";
      lastMove = pipes[lines[x][y]][lastMove];
      switch (lastMove) {
        case LEFT:
          mat[2 * x + 1][2 * y] = "#";
          y--;
    ...
    for (i = 1; i < mat.length; i += 2) {
      for (j = 1; j < mat[i].length; j += 2) {
        if (mat[i][j] === ".") {
          squares++;
        }
      }
