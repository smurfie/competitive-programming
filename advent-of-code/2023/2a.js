(() => {
  function solve(input, red, green, blue) {
    var lines = Utils.read(input);
    var sum = 0;

    for (let line of lines) {
      let game = Number(line.split(": ")[0].split(" ")[1]);
      let draws = line.split(": ")[1].split("; ");
      let possible = true;
      for (let draw of draws) {
        for (let ball of draw.split(", ")) {
          let num = Number(ball.split(" ")[0]);
          let color = ball.split(" ")[1];
          var max = color === "red" ? red : color === "green" ? green : blue;
          if (num > max) {
            possible = false;
          }
        }
