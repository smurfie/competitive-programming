(() => {
    ...
    for (let i = 0; i < lines.length; i++) {
      for (let j = 0; j < lines[i].length; j++) {
        if (lines[i][j] >= "0" && lines[i][j] <= "9") {
          let i1 = i;
          let j1 = j;
          let num = Number(lines[i][j]);
          while (j + 1 < lines[i].length && lines[i][j + 1] >= "0" && lines[i][j + 1] <= "9") {
            j++;
            num *= 10;
            num += Number(lines[i][j]);
          }
          let j2 = j;
          if (isPartNum(lines, i1, j1, j2)) {
            sum += num;
          }
        }
      }
  ...
  function isPartNum(lines, i1, j1, j2) {
    for (let j = j1 - 1; j < j2 + 2; j++) {
      if (isSymbol(lines, i1 - 1, j) || isSymbol(lines, i1 + 1, j)) {
        return true;
      }
    }
    if (isSymbol(lines, i1, j1 - 1) || isSymbol(lines, i1, j2 + 1)) {
      return true;
    }
    return false;
  }

  function isSymbol(lines, i, j) {
    return (
      i >= 0 &&
      i < lines.length &&
      j >= 0 &&
