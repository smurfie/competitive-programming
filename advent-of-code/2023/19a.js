(() => {
  function solve(input) {
    ...
    while (i < lines.length) {
      let objStr = lines[i].substring(1, lines[i].length - 1);
      let arr = objStr.split(",");
      let obj = {};
      for (let value of arr) {
        obj[value.split("=")[0]] = Number(value.split("=")[1]);
      }
      if (accepted(obj, rules)) {
  ...
  function accepted(obj, rules) {
    let workflow = "in";
    while (workflow !== "A" && workflow !== "R") {
      workflow = nextWorkflow(obj, rules[workflow]);
    }
    return workflow === "A";
  }

  function nextWorkflow(obj, rule) {
    let i = 0;
    while (i < rule.length - 1 && !complies(obj, rule[i])) {
      i++;
    }
    return i === rule.length - 1 ? rule[i] : rule[i].split(":")[1];
  }

  function complies(obj, ruleEl) {
