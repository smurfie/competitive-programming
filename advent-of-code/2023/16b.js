(() => {
  ...
  function solve(input) {
    ...
    for (let i = 0; i < map.length; i++) {
      max = Math.max(max, solveConf(map, i, 0, RIGHT));
      max = Math.max(max, solveConf(map, i, map.length - 1, LEFT));
    }

    for (let i = 0; i < map[0].length; i++) {
      max = Math.max(max, solveConf(map, 0, i, DOWN));
      max = Math.max(max, solveConf(map, map[0].length - 1, i, UP));
    }

    return max;
  }

  function solveConf(map, x, y, direction) {
    let map2 = [];
