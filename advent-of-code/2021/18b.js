(() => {
  function solve(input) {
    ...
    for (var i = 0; i < lines.length; i++) {
      for (var j = 0; j < lines.length; j++) {
        if (i != j) {
          var arr = join(JSON.parse(lines[i]), JSON.parse(lines[j]));
          max = Math.max(max, magnitude(arr));
        }
      }
    }
  ...
  function magnitude(arr) {
    var left = isNaN(arr[0]) ? 3 * magnitude(arr[0]) : 3 * arr[0];
    var right = isNaN(arr[1]) ? 2 * magnitude(arr[1]) : 2 * arr[1];
    return left + right;
  }
  ...
  function explodes(arr, n) {
    ...
    for (var i = 0; i < arr.length && !found; i++) {
      ...
      if (isNaN(el)) {
        if (n == 4) {
          ...
          [_, found, addLeft, addRight] = explodes(el, n + 1);
          if (found) {
            if (addLeft > 0) {
              if (i == 1) {
                if (isNaN(arr[0])) {
    ...
    return [arr, found, addLeft, addRight];
  }

  function add(arr, n, i) {
    if (isNaN(arr[i])) {
      add(arr[i], n, i);
    } else {
      arr[i] += n;
