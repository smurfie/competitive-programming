(() => {
  ...
  function getNum(line) {
    ...
    for (var code of codes) {
      if (code.length == 6) {
        if (hasAll(code, ntoc[4])) {
          num = 9;
        } else if (hasAll(code, ntoc[7])) {
          num = 0;
        } else {
          num = 6;
        }
    ...
    for (var code of codes) {
      if (code.length == 5) {
        var num;
        if (hasAll(ntoc[6], code)) {
          num = 5;
        } else if (hasAll(ntoc[9], code)) {
          num = 3;
        } else {
          num = 2;
        }
    ...
    return parseInt(nums.map((i) => cton[i]).join(""));
  }

  function hasAll(more, less) {
    for (var i = 0; i < less.length; i++) {
      if (more.indexOf(less[i]) == -1) return false;
    }
    return true;
  }
...
gcafb gcf dcaebfg ecagb gf abcdeg gaef cafbge fdbac fegbdc | fgae cfgab fg bagce`,
  ...
  dataset.push({
    input: `cbgefad agc fdega cgdf ecdgfa efgca gaefbd edagbc cg ecafb | cdgafbe cfdg cg gac
