(() => {
  function solve(input) {
    ...
    for (let i = 0; i < lines.length; i += 4) {
      let a1 = Number(lines[i].split("+")[1].split(",")[0]);
      let a2 = Number(lines[i].split("+")[2]);
      let b1 = Number(lines[i + 1].split("+")[1].split(",")[0]);
      let b2 = Number(lines[i + 1].split("+")[2]);
      let c1 = offset + Number(lines[i + 2].split("=")[1].split(",")[0]);
      let c2 = offset + Number(lines[i + 2].split("=")[2]);
      sum += best(a1, a2, b1, b2, c1, c2);
    }

    return sum;
  }

  function best(a1, a2, b1, b2, c1, c2) {
    ...
    for (let i = 0; i <= Utils.mcm(a1, b1) && i2 === -1; i++) {
      let tmp = c1 - a1 * i;
      if (tmp >= 0 && tmp % b1 === 0) {
        let j = tmp / b1;
        if (i1 === -1) {
    ...
    if (diff < 0 || remainder < 0 || remainder % diff !== 0) {
      return 0;
    } else {
      times = remainder / diff;
      let i = i1 + times * (i2 - i1);
      let j = (c1 - a1 * i) / b1;
      return 3 * i + j;
    }
  }
