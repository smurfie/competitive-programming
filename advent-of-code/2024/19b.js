(() => {
  function solve(input) {
    ...
    for (let i = 2; i < lines.length; i++) {
      let pattern = lines[i];
      sum += canMakePattern(pattern, towels, dicc);
    }

    return sum;
  }

  function canMakePattern(pattern, towels, dicc) {
    if (dicc[pattern]) {
      return dicc[pattern];
    }
    if (pattern.length === 0) {
    ...
    for (let towel of towels) {
      if (pattern.startsWith(towel)) {
        sum += canMakePattern(pattern.substring(towel.length), towels, dicc);
      }
    }
    dicc[pattern] = sum;
    return sum;
