(() => {
  function solve(input) {
    ...
    for (var line of lines) {
      var p = extract(line.split(", ")[0]);
      var v = extract(line.split(", ")[1]);
      var a = extract(line.split(", ")[2]);
      particles.push({ p, v, a, i });
      i++;
    }
    ...
    for (var i = 0; i < iterations; i++) {
      ...
      for (var j = 0; j < particles.length; j++) {
        ...
        var posx = pos(particle.p[0], particle.v[0], particle.a[0], i);
        var posy = pos(particle.p[1], particle.v[1], particle.a[1], i);
        var posz = pos(particle.p[2], particle.v[2], particle.a[2], i);
        var s = posx + "," + posy + "," + posz;
        if (!dict[s]) dict[s] = [];
        dict[s].push(j);
      }
      var newParticles = [];
      for (var key in dict) {
        if (dict[key].length == 1) newParticles.push(particles[dict[key][0]]);
      }
      var particles = newParticles;
    }
  ...
  function pos(p, v, a, i) {
    return p + v * i + a * ((i * (i + 1)) / 2);
  }
