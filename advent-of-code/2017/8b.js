(() => {
  function solve(input) {
    ...
    for (var line of lines) {
      var register = line.split(" ")[0];
      var inc = parseInt(line.split(" ")[2]);
      if (line.split(" ")[1] == "dec") inc = -inc;

      var command = line.split(" if ")[1];
      command = command.split(" ");
      first = command.shift();
      command.unshift("')");
      command.unshift(first);
      command.unshift("reg(registers,'");
      command = command.join("");
      var b = eval(command);
      if (b) registers[register] = reg(registers, register) + inc;
  ...
  function reg(registers, val) {
    if (!registers[val]) registers[val] = 0;
