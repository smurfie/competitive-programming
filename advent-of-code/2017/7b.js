(() => {
  function solve(input) {
    ...
    for (var line of lines) {
      ...
      var weight = parseInt(parts[0].split(" (")[1]);
      weights[parent] = weight;
      if (parts.length > 1) {
        var childs = parts[1].split(", ");
    ...
    var parent = parents[Object.keys(parents)[0]];
    while (parents[parent]) parent = parents[parent];

    return findUnbalanced(children, weights, parent)[1];
  }

  function findUnbalanced(children, weights, parent) {
    var sum = 0;
    var corrected = 0;
    var childs = children[parent];
    if (childs) {
      var arr = childs.map((i) => findUnbalanced(children, weights, i));
      sum = arr.reduce((a, b) => a + b[0], 0);
      corrected = arr.reduce((a, b) => a + b[1], 0);
      if (sum / arr.length != arr[0][0]) {
        // Correct unbalance
        var correct = arr[0][0] == arr[1][0] ? arr[0][0] : arr[2][0];
        var wrongId = arr.findIndex((i) => i[0] != correct);

        var diff = correct - arr[wrongId][0];
        weights[childs[wrongId]] += diff;
        corrected = weights[childs[wrongId]];
        sum += diff;
      }
    }
    return [sum + weights[parent], corrected];
  }
