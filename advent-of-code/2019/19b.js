(() => {
  function solve(input) {
    // Start at 100 for example, and stop when reaching 199 spaces for our ship to fit
    var dict = {};
    for (var i = 100; ; i++) {
      var el = "#";
      // The beam always starts at i+1 or more but check it just in case
      if (runProgram(input, i, i) == "#") {
        console.log("error!");
      }
      var start = 0;
      var end = 0;
      for (var j = i + 1; start == 0 || end == 0; j++) {
        var el = runProgram(input, i, j) == 0 ? "." : "#";
        if (start == 0 && el == "#") start = j;
        if (start != 0 && end == 0 && el == ".") end = j;
      }
      // Second check
      if (runProgram(input, i, i + 300) == "#") {
        console.log("error!");
      }
      dict[i] = [start, end - 1];
      if (i >= 200 && dict[i - 99][1] >= start + 99)
        return 10000 * (i - 99) + start;
    }
  }
