  ...
  function solve(input) {
    ...
    while (opcode != 99) {
      ...
      if (opcode == 1) {
        ...
        var output = number(program, program[i + 1], mode1, relBase);
        if (outputColor) {
          if (dict[x + "," + y] == undefined) paints++;
          if (dict[x + "," + y] != undefined || output == 1) {
            matrix[x][y] = output == 0 ? "." : "#";
          }
          dict[x + "," + y] = output;
        } else {
    ...
    // Uncomment next line to see the result
