(() => {
  function solve(input) {
    ...
    var wire0 = lines[0].split(",");
    var wire1 = lines[1].split(",");
    ...
    var steps = 1;
    var min = Infinity;
    for (var path of wire0) {
      var dir = path[0];
      var l = parseInt(path.substring(1));
      while (l-- > 0) {
        switch (dir) {
          case "U":
            y++;
        ...
        if (!dict[x + "," + y]) dict[x + "," + y] = steps++;
      }
    }
    x = 0;
    y = 0;
    steps = 1;
    for (var path of wire1) {
      var dir = path[0];
      var l = parseInt(path.substring(1));
      while (l-- > 0) {
        switch (dir) {
        ...
        if (dict[x + "," + y]) min = Math.min(min, dict[x + "," + y] + steps);
        steps++;
      }
    }
