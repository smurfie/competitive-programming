...
function run(n, m, start, base) {
  ...
  while (next != "AC" && next != "F") {
    ...
    if (notVisited.length > 0) chooseBest2(notVisited);
...
function chooseBest2(arr) {
  var arr2 = arr.filter((i) => i[0] == 4);
  if (arr2.length > 0) print(arr2[0][2]);
  else {
    arr.sort((a, b) => a[0] - b[0]);
    arr = arr.filter((i) => i[0] == arr[0][0]);
    print(arr[Math.floor(Math.random() * arr.length)][2]);
  }
}
